...
public:
	...
	static void convert_to_hex_string(std::ostringstream& op, const unsigned char* data, int size);

	static void convert_to_hex_slConfig(std::ostringstream& op, const unsigned char* data, int size);

	static void convert_to_struct(std::istream& ip, unsigned char* data, int size);

	static pugi::xml_node parameterSet(pugi::xml_node docw, const char* jebo, const char* jebo2);

	static pugi::xml_node parameterSetComm(pugi::xml_node docw, const char* jebo);

	static void convert_to_hex_stringTest(std::ostringstream& op, const unsigned char* data, int size);

	static void convert_to_structTest(std::istream& ip, unsigned char* data, int size);

	static uint32_t crc32Checksum(uint32_t len, const uint8_t* pBuffer, uint32_t initCRC)
	{
	...
	static bool UploadFile(const std::string& filePath, const std::string& ftpServer, const std::string& userName, const std::string& password);
	static std::ostringstream noNewline;
};
